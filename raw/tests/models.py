...
# =========================
# EXOTIC COVWMA AND MACD MODELS
# =========================
def exotic_covwma(prices, period=12):
    """Covariance-weighted moving average with volatility and momentum adjustments."""
    try:
        prices = np.array(prices, dtype=np.float64)
        prices = np.where(np.isfinite(prices), prices, np.nan)
        prices = pd.Series(prices).fillna(method="ffill").fillna(method="bfill").values
...
def exotic_macd(prices, fast=12, slow=26, signal=9):
    """MACD-based forecast with momentum & mean-reversion adjustment."""
    try:
        prices = np.array(prices, dtype=np.float64)
        prices = np.where(np.isfinite(prices), prices, np.nan)
        prices = pd.Series(prices).fillna(method="ffill").fillna(method="bfill").values
        ...
        while True:
            ...
            for ex, price in results:
                if price:
                    ...
                    preds = {name: func(list(history[ex])) for name, func in MODELS.items()}
                    ...
                    print(f"{ex:10} | Price: {price:10.2f} | " +
                          " | ".join([f"{k}: {v:.2f}" if v else f"{k}: waiting" for k,v in preds.items()]))
