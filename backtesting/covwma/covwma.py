...
# COVWMA MODEL
# =========================
def covwma_vectorized(prices, max_period=1000):
...
# Only use the top periods for trading
BEST_PERIODS = [26, 38, 27, 25, 23, 36, 28, 22, 35, 37]

def predict_covwma_vectorized(prices):
    if len(prices) < max(BEST_PERIODS):
        return None
    prices = np.array(prices, dtype=np.float64)
    all_ma = covwma_vectorized(prices, max_period=max(BEST_PERIODS))
    # Take the last value of each best period and average them
    last_vals = [all_ma[p-1, -1] for p in BEST_PERIODS]
    forecast = np.nanmean(last_vals)
    return float(forecast) if not np.isnan(forecast) else None

# Add to MODELS
MODELS["CovWMA"] = predict_covwma_vectorized
        ...
        while True:
            ...
            for ex, price in results:
                if price:
                    ...
                    pred = MODELS["CovWMA"](list(history[ex])) if len(history[ex]) >= 12 else None
